...
class RawDataValidation:
    """Handles all the validation done on the raw training data
    """
    def __init__(self, path):
        self.batch_directory = path
        self.schema_path = 'schema_training.json'
        self.logger = create_log('Training_Logs/values_from_schema_validation', filemode='a+')
        self.general_logger = create_log("Training_Logs/general", filemode="a+")
        self.name_validation_logger = create_log("Training_Logs/name_validation", filemode="a+")
        self.column_validation_logger = create_log("Training_Logs/column_validation", filemode="a+")
        self.missing_value_logger = create_log("Training_Logs/missing_values_in_column", filemode="a+")
    ...
    def move_bad_files_to_archive_bad(self):
        ...
        try:
            ...
            if os.path.isdir(source):
                ...
                if not os.path.isdir(path):
                    os.makedirs(path)
                destination = 'Training_Archive_Bad_Data/BadData_' + str(date)+"_"+str(time)
    ...
    def validation_file_name_raw(self, regex, LengthOfDateStampInFile, LengthOfTimeStampInFile):
        """Validates the name of the training csv files as per given name in the schema
