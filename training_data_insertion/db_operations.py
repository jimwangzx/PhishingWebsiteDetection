...
class DBOperation:
    ...
    def __init__(self):
        ...
        self.data_base_connection_logger = create_log("Training_Logs/data_base_connection", filemode="a+")
        self.db_table_create_logger = create_log("Training_Logs/db_table_create", filemode="a+")
        self.db_insert_logger = create_log("Training_Logs/db_insert", filemode="a+")
        self.db_export_csv_logger = create_log("Training_Logs/export_to_csv", filemode="a+")
    ...
    def data_base_connection(self, keyspace_name):
        ...
        try:
            ...
            self.data_base_connection_logger.info('Failure can be due to hibernated data.')
            self.data_base_connection_logger.info('So the issue can be solved by running \
            the code after 1-5 minutes')
    ...
    def create_table_db(self, keyspace_name):
        ...
        """
        try:
            data = pd.read_csv('Training_Batch_Files/phishing_09032022_ 082200.csv')
            ...
            try:
                create_query = f"CREATE TABLE phishing1 ({column_sequence1});"
                session.execute(create_query)

                create_query = f"CREATE TABLE phishing2 ({column_sequence2});"
                session.execute(create_query)
    ...
    def insert_into_table_good_data(self, keyspace_name):
        ...
        try:
            data1 = pd.DataFrame(session.execute('SELECT * FROM "phishing1";').all())
            data2 = pd.DataFrame(session.execute('SELECT * FROM "phishing2";').all())
            ...
            for file in only_files:
                try:
                    ...
                    for i in tqdm(range(1, len(lines))):
                        ...
                        insert_query1 = f"INSERT INTO phishing1 ({column_names1}) VALUES({column_values1});"
                        session.execute(insert_query1)

                        insert_query2 = f"INSERT INTO phishing2 ({column_names2}) VALUES({column_values2});"
